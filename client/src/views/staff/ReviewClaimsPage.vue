...
// Metrics Computation
const totalCount = computed(() => pagination.value?.total || claims.value.length)
...
const rejectedCount = computed(() => claims.value.filter(c => c.status === 'rejected').length)
...
async function load(page = 1) {
  ...
  try {
    ...
    uiStore.error('Failed to load claims')
  }
}
...
async function handleRefresh() {
  ...
  try {
    ...
    uiStore.success('Claims list refreshed')
  } catch {
...
function exportClaimsCsv() {
  ...
  const headers = ['Claim ID', 'Item Name', 'Item Ref', 'Claimant Name', 'Date Filed', 'Status']
  ...
  <div class="space-y-4 sm:space-y-5" @click="closeClaimMenu">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      ...
      <div class="p-3.5 sm:p-4 rounded-xl bg-white dark:bg-[#111827] border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center justify-between">
        ...
        <div class="h-10 w-10 sm:h-11 sm:w-11 rounded-xl bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 flex items-center justify-center font-bold">
    ...
    <!-- Enterprise Review Claims Data Table -->
      ...
      <table class="w-full min-w-[800px] text-sm">
        <thead>
          <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 text-xs">
            ...
            <th class="w-24 px-4 py-3.5 text-center font-bold uppercase tracking-wider">
              {{ t('common.actions') }}
            </th>
          </tr>
        </thead>
        <tbody v-if="loading && claims.length === 0" class="divide-y divide-slate-100 dark:divide-slate-800">
          <tr v-for="n in 4" :key="n" class="animate-pulse">
            ...
            <td class="px-4 py-4 text-center">
              <div class="h-8 w-20 bg-slate-200 dark:bg-slate-800 rounded-xl mx-auto" />
            </td>
          </tr>
        </tbody>
            ...
            <td colspan="7" class="p-12 text-center">
              <div class="max-w-xs mx-auto space-y-2 text-center">
                ...
                <p class="font-bold text-slate-800 dark:text-slate-200 text-sm">
                  {{ searchQuery || selectedStatus !== 'all' ? 'No claims match your active filters' : 'No claims under review' }}
                </p>
                <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  {{ searchQuery || selectedStatus !== 'all' ? 'Try adjusting your search criteria or status filter.' : 'All student ownership claims submitted across campus facilities will appear here for staff verification and adjudication.' }}
                </p>
              </div>
            </td>
          </tr>
          ...
          >
            ...
            <td class="px-4 py-3.5">
              <div>
                ...
                <p v-if="claim.item?.reference_code" class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 font-mono">
                  Ref: {{ claim.item.reference_code }}
                </p>
              </div>
            </td>
            ...
            <td class="px-4 py-3.5 text-slate-800 dark:text-slate-200 font-medium">
              {{ claim.claimant?.full_name || 'Student Claimant' }}
            </td>
            ...
            <td class="px-4 py-3.5 text-center">
              <div class="flex items-center justify-center gap-1.5">
                <button
                  type="button"
                  :class="[
                    'h-8 px-2.5 rounded-xl text-xs font-semibold inline-flex items-center gap-1 transition-colors cursor-pointer',
                    claim.status === 'approved'
                      ? 'bg-[#0B5D3B] text-white shadow-2xs hover:bg-[#09482e]'
                      : 'bg-amber-500 text-white shadow-2xs hover:bg-amber-600',
                  ]"
                  @click="claim.status === 'approved' ? goToProcessReturn(claim) : openReview(claim)"
                >
                  <Handshake v-if="claim.status === 'approved'" class="h-3.5 w-3.5 mr-0.5" />
                  <CheckSquare v-else class="h-3.5 w-3.5 mr-0.5" />
                  <span>{{ claim.status === 'approved' ? 'Handover' : 'Adjudicate' }}</span>
                </button>

                <AppActionMenu v-slot="{ close }" width-class="w-52">
                  <!-- 1. Review Ownership Proof -->
                  <button
                    type="button"
                    ...
                    @click="close(); openReview(claim)"
                  >
                    <Eye class="h-4 w-4 text-slate-400" />
                    <span>{{ claim.status === 'approved' ? 'Re-evaluate Claim' : 'Review Ownership Proof' }}</span>
                  </button>
                  ...
                  <button
                    ...
                    type="button"
                    class="w-full text-left px-3.5 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 flex items-center gap-2.5 font-medium transition-colors cursor-pointer"
                    @click="close(); goToProcessReturn(claim)"
                  >
                    <Handshake class="h-4 w-4 text-emerald-500" />
