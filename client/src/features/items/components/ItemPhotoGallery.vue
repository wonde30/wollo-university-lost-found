...
<template>
  <div class="space-y-3">
    <!-- Main Photo Display -->
    <div class="relative h-72 sm:h-96 w-full rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 shadow-xs flex items-center justify-center">
      <img
        v-if="activePhotoUrl"
        :src="activePhotoUrl"
        alt="Item photo"
      ...
      />
      <div v-else class="text-center text-slate-400 p-8">
        ...
        <span class="text-xs font-medium">No photos uploaded</span>
      </div>
    </div>
    ...
    <div v-if="photos.length > 1" class="flex gap-2 overflow-x-auto py-1">
      <button
        ...
        :class="[
          ...
          selectedIndex === index ? 'border-[#0F5132] ring-2 ring-[#0F5132]/30 scale-95' : 'border-slate-200 opacity-70 hover:opacity-100',
        ]"
        @click="selectedIndex = index"
      >
        <img :src="resolveStorageUrl(p.photo_url)" alt="Thumbnail" class="h-full w-full object-cover" />
      </button>
    </div>
  </div>
</template>
